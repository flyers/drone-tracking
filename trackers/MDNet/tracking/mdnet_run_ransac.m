function [ result ] = mdnet_run_ransac(images, region, net, display)
% MDNET_RUN
...
img = imread(images{1});
scale_height = size(img, 1) / normal_height;
scale_width = size(img, 2) / normal_width;
if(size(img,3)==1), img = cat(3,img,img,img); end
targetLoc = region;
if useNormalSize
    img = imresize(img, [normal_height, normal_width]);
    targetLoc = targetLoc ./ [scale_width, scale_height, scale_width, scale_height];
else
    scale_width = 1;
    scale_height = 1;
end
...
camera_feat.last_keypoints = camera_feat.detector.detect(img);
camera_feat.last_descriptors = camera_feat.extractor.compute(img, camera_feat.last_keypoints);
...
for To = 2:nFrames;
    ...
    [camera_feat, camera_H] = find_homography(camera_feat, img, targetLoc([2,1]) + targetLoc([4,3])/2, targetLoc([4,3]));
    targetLoc(1:2) = project_t(targetLoc(1:2), camera_H);
    targetLoc(1:2) = max([0,0], targetLoc(1:2));
    targetLoc(1:2) = min([size(img,2),size(img,1)]-targetLoc(3:4), targetLoc(1:2));
...
%         fprintf('\n');
    ...
    result(To,:) = result(To,:) .* [scale_width, scale_height, scale_width, scale_height];
end
