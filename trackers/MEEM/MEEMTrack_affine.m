...
function result = MEEMTrack_affine(input, ext, show_img, init_rect, start_frame, end_frame, seq)
...
%     I_orig=imread(fullfile(input,file_list{frame_id}));
    ...
    if frame_id == start_frame
        ...
        [feat, H] = find_affine(feat, I_scale, output([2,1]) + output([4,3])/2, output([4,3]));
