...
function result = MEEMTrack_ransac(input, ext, show_img, init_rect, start_frame, end_frame, seq)
...
%     I_orig=imread(fullfile(input,file_list{frame_id}));
    I_orig=imread(seq.s_frames{count});
    ...
    if frame_id == start_frame
        ...
        feat.last_keypoints = feat.detector.detect(I_scale);
        feat.last_descriptors = feat.extractor.compute(I_scale, feat.last_keypoints);
    else
        [feat, H] = find_homography(feat, I_scale, output([2,1]) + output([4,3])/2, output([4,3]));
        output(1:2) = project_t(output(1:2), H);
        svm_tracker.output(1:2) = project_t(svm_tracker.output(1:2), H);
        % out of boundary case
        output(1:2) = min(output(1:2), [size(I_scale, 2), size(I_scale, 1)]);
        svm_tracker.output(1:2) = min(svm_tracker.output(1:2), [size(I_scale, 2), size(I_scale, 1)]);
        svm_tracker.output_exp(1:2) = project_t(svm_tracker.output_exp(1:2), H);
