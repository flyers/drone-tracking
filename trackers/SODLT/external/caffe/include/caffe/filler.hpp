...
template <typename Dtype>
class SparseFiller : public Filler<Dtype> {
   public:
     explicit SparseFiller(const FillerParameter& param):Filler<Dtype>(param) {}
     virtual void Fill(Blob<Dtype>* blob) {
        Dtype * data=blob->mutable_cpu_data();
	      CHECK(blob->count());
	      caffe_vRngGaussian<Dtype>(blob->count(), blob->mutable_cpu_data(),
          Dtype(this->filler_param_.mean()),
          Dtype(this->filler_param_.std()));
	      int rK=this->filler_param_.random_k();
        vector<int> order;
        ...
        for (int i = 0; i < width; ++i){
          ...
          random_shuffle(order.begin(), order.end());
        	...
        	for(int i = 0; i < width - rK; ++i){
            data_vec[h * width + order[i]] = 0.0;
	        }
        }
     }	
};
...
Filler<Dtype>* GetFiller(const FillerParameter& param) {
  ...
  if (type == "constant") {
    ...
    return new SparseFiller<Dtype>(param);
  }else {
