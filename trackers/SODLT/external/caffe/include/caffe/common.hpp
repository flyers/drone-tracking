...
class Caffe {
 public:
  ...
  inline static cublasHandle_t cublas_handle(int i = 0) { return Get().cublas_handle_[i]; }

  // Returns the stream handle.
  inline static cudaStream_t stream_handle(int i) {return Get().stream_handle_[i];}
 ...
 protected:
  cublasHandle_t cublas_handle_[16];
  cudaStream_t stream_handle_[16];
