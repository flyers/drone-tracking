...
template <typename Dtype>
void Blob<Dtype>::ReshapeNoErase(const int num, const int channels, const int height,
    const int width) {
  CHECK_GE(num, 0);
  CHECK_GE(channels, 0);
  CHECK_GE(height, 0);
  CHECK_GE(width, 0);
  CHECK_EQ(num * channels * height * width, count_);
  num_ = num;
  channels_ = channels;
  height_ = height;
  width_ = width;
...
template <typename Dtype>
void Blob<Dtype>::Update(int streamId) {
  ...
  case SyncedMemory::SYNCED:
    ...
    caffe_gpu_axpy<Dtype>(count_, Dtype(-1),
        ...
        reinterpret_cast<Dtype*>(data_->mutable_gpu_data()), streamId);
    break;
  default:
