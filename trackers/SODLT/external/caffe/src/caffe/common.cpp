...
Caffe::Caffe()
    : mode_(Caffe::GPU), phase_(Caffe::TRAIN),
...
Caffe::~Caffe() {
  if (cublas_handle_[0]){
    for (int i = 0; i < 16; ++i){
      CUBLAS_CHECK(cublasDestroy(cublas_handle_[i]));
    }
  }
...
void Caffe::SetDevice(const int device_id) {
  ...
  if (current_device == device_id && Get().cublas_handle_[0]
    && Get().curand_generator_) {
    return;
  }
  if (Get().cublas_handle_[0]){
    for (int i = 0; i < 16; ++i){
  ...
  for (int i = 0; i < 16; ++i){
    CUDA_CHECK(cudaStreamCreate(&Get().stream_handle_[i]));
  }
  for (int i = 0; i < 16; ++i){
    cublasSetStream(Caffe::cublas_handle(i), Caffe::stream_handle(i));
  }
  CURAND_CHECK(curandCreateGenerator(&Get().curand_generator_,
      CURAND_RNG_PSEUDO_DEFAULT));
