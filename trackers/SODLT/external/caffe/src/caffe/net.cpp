...
void Net<Dtype>::Init(const NetParameter& param) {
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    // Send this pointer to each layer.
    layers_[layers_.size() - 1]->SetNet(this);
    ...
    if(layers_[i]->blobs().size())
      LOG(INFO)<<layer_param.name()<<" "<<layers_[i]->blobs()[0]->num();
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      CHECK_EQ(source_layer.blobs(j).num(), target_blobs[j]->num());
...
template <typename Dtype>
void Net<Dtype>::Update(int streamNum) {
  for (int i = 0; i < params_.size(); ++i) {
    params_[i]->Update(i % streamNum);
  }
...
template <typename Dtype>
inline const string& Net<Dtype>::name() { return name_; }
// returns the layer names
template <typename Dtype>
inline const vector<string>& Net<Dtype>::layer_names() { return layer_names_; }
// returns the blob names
template <typename Dtype>
inline const vector<string>& Net<Dtype>::blob_names() { return blob_names_; }
// returns the blobs
template <typename Dtype>
inline const vector<shared_ptr<Blob<Dtype> > >& Net<Dtype>::blobs() { return blobs_; }
// returns the layers
template <typename Dtype>
inline const vector<shared_ptr<Layer<Dtype> > >& Net<Dtype>::layers() { return layers_; }
...
template <typename Dtype>
inline vector<vector<Blob<Dtype>*> >& Net<Dtype>::bottom_vecs() { return bottom_vecs_; }
template <typename Dtype>
inline vector<vector<Blob<Dtype>*> >& Net<Dtype>::top_vecs() { return top_vecs_; }
// returns the parameters
template <typename Dtype>
inline vector<shared_ptr<Blob<Dtype> > >& Net<Dtype>::params() { return params_; }
// returns the parameter learning rate multipliers
template <typename Dtype>
inline vector<float>& Net<Dtype>::params_lr() {return params_lr_; }
template <typename Dtype>
inline vector<float>& Net<Dtype>::params_weight_decay() { return params_weight_decay_; }
// Input and output blob numbers
template <typename Dtype>
inline int Net<Dtype>::num_inputs() { return net_input_blobs_.size(); }
template <typename Dtype>
inline int Net<Dtype>::num_outputs() { return net_output_blobs_.size(); }
template <typename Dtype>
inline vector<Blob<Dtype>*>& Net<Dtype>::input_blobs() { return net_input_blobs_; }
template <typename Dtype>
inline vector<Blob<Dtype>*>& Net<Dtype>::output_blobs() { return net_output_blobs_; }
template <typename Dtype>
inline vector<int>& Net<Dtype>::input_blob_indices() { return net_input_blob_indices_; }
template <typename Dtype>
inline vector<int>& Net<Dtype>::output_blob_indices() { return net_output_blob_indices_; }
// For adaptive batch size
template <typename Dtype>
int Net<Dtype>::GetBatchSize() { return batch_size_; }
template <typename Dtype>
void Net<Dtype>::SetBatchSize(int batchSize) { batch_size_ = batchSize; }
