...
void ConvolutionLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(NUM_OUTPUT_ % MULTIPLY_BATCH, 0) << "Filter number should be dividable by multiply_batch.";
  ...
  col_buffer_.Reshape(MULTIPLY_BATCH, CHANNELS_ * KSIZE_ * KSIZE_, height_out, width_out);
  // Set the parameters
...
void ConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int weight_offset = M_ * K_;
  int col_offset = K_ * N_ * MULTIPLY_BATCH;
  int top_offset = M_ * N_ * MULTIPLY_BATCH;
  int height_out = (HEIGHT_ + 2 * PAD_ - KSIZE_) / STRIDE_ + 1;
  int width_out = (WIDTH_ + 2 * PAD_ - KSIZE_) / STRIDE_ + 1;
  for (int n = 0; n < NUM_ / MULTIPLY_BATCH; ++n) {
    // First, im2col
    for (int i = 0; i < MULTIPLY_BATCH; ++i){
      im2col_gpu(bottom_data + bottom[0]->offset(n * MULTIPLY_BATCH + i), CHANNELS_, HEIGHT_,
    ...
    for (int g = 0; g < GROUP_; ++g) {
      ...
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, K_,
        (Dtype)1., weight + weight_offset * g, col_data + col_buffer_.offset(i) + col_offset * g,
        (Dtype)0., top_data + (*top)[0]->offset(n * MULTIPLY_BATCH + i) + top_offset * g, i);
      }
    }
    // third, add bias
    if (biasterm_) {
    ...
    for (int g = 0; g < GROUP_; ++g) {
      for (int i = 0; i < MULTIPLY_BATCH; ++i){
        caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, M_, K_, N_,
          (Dtype)1., top_diff + top[0]->offset(n * MULTIPLY_BATCH + i) + top_offset * g,
          col_data + col_buffer_.offset(i) + col_offset * g, (Dtype)1.,
          weight_diff + weight_offset * g, i);
      }
    }
    // gradient w.r.t. bottom data, if necessary
    if (propagate_down) {
      for (int g = 0; g < GROUP_; ++g) {
        ...
        caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, K_, N_, M_,
          (Dtype)1., weight + weight_offset * g,
          top_diff + top[0]->offset(n * MULTIPLY_BATCH + i) + top_offset * g,
          (Dtype)0., col_diff + col_buffer_.offset(i) + col_offset * g, i);
        }
      }
    }
      // col2im back to the data
    for (int i = 0; i < MULTIPLY_BATCH; ++i){
      col2im_gpu(col_diff + col_buffer_.offset(i), CHANNELS_, HEIGHT_,
      ...
      for (int j = 0; j < MULTIPLY_BATCH; ++j){
        ...
        if (ratio < 1){
          caffe_gpu_scal(CHANNELS_ / GROUP_ * KSIZE_ * KSIZE_, ratio, 
            this->blobs_[0]->mutable_gpu_data() + this->blobs_[0]->offset(i * MULTIPLY_BATCH + j), j);
        }
      }
    }
    cudaDeviceSynchronize();
