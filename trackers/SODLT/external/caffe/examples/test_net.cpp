...
int main(int argc, char** argv) {
  if (argc < 6) {
    LOG(ERROR) << "test_net net_proto pretrained_net_proto iterations [VAL/TEST] [CPU/GPU] [GPU Device Id]";
    return 0;
  }
  
  if (strcmp(argv[4], "VAL") == 0){
    Caffe::set_phase(Caffe::VAL);
    LOG(INFO) << "Set to validation mode.";
  } else {
    Caffe::set_phase(Caffe::TEST);
    LOG(INFO) << "Set to test mode.";
  }
