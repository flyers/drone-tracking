...
int main(int argc, char** argv) {
  ...
  options.write_buffer_size = 2684354;
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
	...
	//cout<<root_folder+lines[line_id].first<<endl;
    /*if (!data_size_initialized) {
      data_size = datum.channels() * datum.height() * datum.width();
      data_size_initialized = true;
    } else {
      const string& data = datum.data();
      CHECK_EQ(data.size(),data_size) << root_folder+lines[line_id].first+"Incorrect data field size " << data.size();
    }*/
	min_len=datum.height();
	if(datum.width()<min_len)
		min_len=datum.width();
	CHECK_EQ(min_len,256)<< root_folder+lines[line_id].first+"Incorrect data field size " << datum.data().size();
    // sequential
