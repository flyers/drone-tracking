function [positions, fps] = dsst_ransac(params)
%% Modified to correct for camera motion by projective transformation
% parameters
...
feat.last_keypoints = feat.detector.detect(im);
feat.last_descriptors = feat.extractor.compute(im, feat.last_keypoints);
% H_res = zeros(3,3,num_frames-1);
% aff_res = zeros(6,num_frames-1);
...
for frame = 1:num_frames,
    ...
    if frame > 1
        % note the center position pos(1) is height, pos(2) is width, same for target_sz
        [feat, H] = find_homography(feat, im, pos, target_sz);
        % H_res(:,:,frame-1) = H;
        % affparam = (affparam2geom( [H(1,3), H(2,3), H(1,1), H(1,2), H(2,1), H(2,2)]' ));
        % aff_res(:,frame-1) = affparam;
        H_p = zeros(3,3,2);
        H_p(:,:,1) = H;
        H_p(:,:,2) = eye(3);
%         if frame > 2
%             % disp( H_res(:,:,frame-1) - H_res(:,:,frame-2) );
% %             disp(aff_res(:, frame-1) - aff_res(:, frame-2));
%         end
    end
    ...
    if frame > 1
        for i = 1:1
            ...
            tracker_response(i, :) = max(response(:));
            ...
            % update the scale
            tracker_scale(i) = currentScaleFactor * scaleFactors(recovered_scale);
            if tracker_scale(i) < min_scale_factor
                tracker_scale(i) = min_scale_factor;
            elseif tracker_scale(i) > max_scale_factor
                tracker_scale(i) = max_scale_factor;
            end
        end
        [~, odx] = max(tracker_response);
        pos = tracker_pos(odx, :);
        currentScaleFactor = tracker_scale(odx);
    end
...
% dif = zeros(size(H_res));
% dif = zeros(size(aff_res));
% for i = 1:size(H_res, 3)-1
	% dif(:,:,i) = H_res(:,:,i+1) - H_res(:,:,i);
    % dif(:, i) = aff_res(:, i+1) - aff_res(:, i);
% end
