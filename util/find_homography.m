function [feat, H] = find_homography(feat, im, target_pos, target_size)
%% estimate the projective transformation from the previous image to current image
...
if size(last_pts, 1) < 50
	...
	[H, inliers] = cv.findHomography(last_pts, cur_pts, 'Method','Ransac');
	if isempty(H)
		H = eys(3);
	end
end
