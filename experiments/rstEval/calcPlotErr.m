...
for index_seq=1:length(seqs)
    ...
    fileName = fullfile(pathAnno, [seq_name, '.txt']);
...
%         legend(trackerNames,'Position', 'Best');
        ...
        print(h1, '-depsc', fullfile(pathPlot, [seq_name, '_coverage']));
        imwrite(frame2im(getframe(h1)), fullfile(pathPlot, [seq_name, '_coverage.png']));
...
%         legend(trackerNames,'Position', 'Best');
        ...
        print(h2, '-depsc', fullfile(pathPlot, [seq_name, '_center']));
        imwrite(frame2im(getframe(h2)), fullfile(pathPlot, [seq_name, '_center.png']));
